#include "hashArray.h"
#include "../pVoidArray/pVoidArray.h"
...
)
{
    ...
    for (size_t i = 0; i < chain->size; i += 1) {
        ...
        if (vu_memcmp(e->key, key, keyLen) != 0) continue;
...
static int8_t bucketsNew(pVoidArray** out, size_t count,
    ...
    outcode = pVoidArray_new_hidden(&buckets, file, line, func);
    if (outcode != PV_OUTCODE_OK) {
        return fromPv(outcode);
    }
...
static int8_t placeEntry(pVoidArray* buckets, size_t capacity, hashArrayElement* e,
    ...
    if (!chain) {
        outcode = pVoidArray_new_hidden(&chain, file, line, func);
        if (outcode != PV_OUTCODE_OK) {
            return fromPv(outcode);
        }
...
static int8_t insert(hashArray* table, const void* key, size_t keyLen,
    ...
    line = getLine(table);
    func = getFunc(table);
    e = (hashArrayElement *)memallocate_debug
    (
...
/*
    Тяжёлая операция.
    Разворачивает всю таблицу в один непрерывный блок hashArrayPair:
    O(capacity + size) по времени, одна аллокация на size элементов.
    Снимок НЕ владеющий: key/value - те же указатели, что лежат в таблице.
    Освобождать через hashArray_flat_delete.
    Порядок: по возрастанию номера бакета, внутри бакета - порядок цепочки.
*/
static int8_t flatten(const hashArray* table, hashArrayPair** out, size_t* outCount) {
    ...
    pairs = (hashArrayPair *)memallocate_debug(
        sizeof(hashArrayPair) * table->size, getFile(table);, getLine(table), getFunc(table));
    if (!pairs) {
        return HA_OUTCODE_ALLOC_ERROR;
    }
    vu_memset(pairs, 0, sizeof(hashArrayPair) * table->size);
    ...
    for (size_t i = 0; i < buckets->size; i += 1) {
        ...
        for (size_t j = 0; j < chain->size; j += 1) {
            ...
            if (used >= table->size) {
                /* рассинхрон size и содержимого бакетов */
                memfree(pairs);
                return HA_OUTCODE_UNKNOWN_ERROR;
            }
...
int8_t hashArray_new_hidden(hashArray** table, size_t initialCapacity,
    ...
    (*table) = memallocate_debug(
        sizeof(hashArray), file, line, func);

    if (!*table) {
        return HA_OUTCODE_ALLOC_ERROR;
    }

    outcode = bucketsNew(&buckets, initialCapacity, file, line, func);
    if (outcode != HA_OUTCODE_OK) {
        memfree(*table);
        *table = NULL;
        return outcode;
